...
Treat skill authoring as part of Hermes Agent extension work, not as a separate one-off niche. For new or edited skills, use class-level triggers, peer-matched frontmatter, support files under `references/`, `templates/`, `scripts/`, or `assets/`, and validator-safe YAML. The former `hermes-agent-skill-authoring` package is preserved at `references/original-skills/hermes-agent-skill-authoring/SOURCE_SKILL.md` for the full authoring checklist.
```
